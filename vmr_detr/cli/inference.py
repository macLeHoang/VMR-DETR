...
from vmr_detr.config.options import TestOptions
...
def setup_model(opt):
    ...
    if cosine_T0 > 0:
        ...
        logger.info(f"Using StepLR fallback (lr_drop={opt.lr_drop}).")

    if opt.resume is not None:
        logger.info(f"Load checkpoint from {opt.resume}")
        # PyTorch >= 2.6 defaults to weights_only=True, but our checkpoint also stores
        ...
        if opt.resume_all and "model_raw" in checkpoint:
