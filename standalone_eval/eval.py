...
def _zero_iou_scores(iou_thds, include_average=True):
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    scores = {str(e): 0.0 for e in iou_thds}
    if include_average:
        scores["average"] = 0.0
    return scores


def _zero_mr_metrics():
    return {
        "MR-mIoU": 0.0,
        "MR-mAP": _zero_iou_scores(np.linspace(0.5, 0.95, 10)),
        "MR-R1": _zero_iou_scores(np.linspace(0.3, 0.95, 14), include_average=False),
    }
...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
    ...
    if len(ground_truth) == 0:
        return _zero_iou_scores(iou_thds)
    ...
    data_triples = [[qid, gt_qid2data[qid], pred_qid2data[qid]] for qid in gt_qid2data]
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        percent = 100 * len(_ground_truth) / len(ground_truth) if len(ground_truth) > 0 else 0.0
        print(f"{name}: {l_range}, {len(_ground_truth)}/{len(ground_truth)}="
              f"{percent:.2f} examples.")
        if len(_ground_truth) == 0:
            ret_metrics[name] = _zero_mr_metrics()
        else:
